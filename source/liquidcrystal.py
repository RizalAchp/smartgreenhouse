...
class LiquidCrystal_I2C:
    ...
    def __init__(self, address: int, column: int, row: int, board: telemetrix.Telemetrix, dotsize: int = 1) -> None:
        ...
        self.row: int = row
        if not isinstance(board, telemetrix.Telemetrix):
            raise AttributeError("argument board bukan dari telemetrix.Telemetrix")
        else:
            self.board: telemetrix.Telemetrix = board
